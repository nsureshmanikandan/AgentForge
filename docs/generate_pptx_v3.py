"""
AgentForge — Enterprise Leadership Presentation V3.0
Generates: AgentForge-PresentationV3.0.pptx
Run: python generate_pptx_v3.py
"""
...
box(sl, 0.65, 0.5, 7, 0.5, "ENTERPRISE AI AGENT PLATFORM  ·  v3.0",
...
box(sl, 0.65, 2.5, 8.5, 0.6,
    "Build · Orchestrate · Govern · Voice-Enable · Evaluate Production AI Agents",
...
pills_data = [
    ("17 Enterprise Features",   RGBColor(0x12, 0x30, 0x60), C_BLUE_ACC),
...
for i, (txt, bg, fg) in enumerate(pills_data):
    pill(sl, 0.65 + i*3.17, 3.82, 3.0, 0.52, txt, bg, fg, 13)
...
box(sl, 0.65, 4.95, 12, 0.42,
    "Python 3.12  ·  FastAPI  ·  SQLAlchemy  ·  LangChain  ·  Presidio  ·  PostgreSQL + pgvector  ·  Azure OpenAI  ·  Azure Speech",
    size=13, color=RGBColor(0x50, 0x62, 0x80), font=FONT_BODY)
...
box(sl, 0.55, 1.42, 12, 0.6,
    ...
    "test, voice-enable, and govern intelligent AI automation — visually, without deep ML expertise, "
...
pts = [
    ...
    (C_GREEN_ACC,  "Solution",
     "AgentForge delivers drag-drop visual agent building, GPT-4o prompt-to-agent generation, "
     "built-in Safety & Guardrails, Evaluation engine, Voice Agents via Azure Speech, "
     "and a full observability control plane — out of the box."),
    (C_AMBER_ACC,  "Business Impact",
     "10× faster agent creation  ·  Zero compliance exposure via always-on PII/safety rules  ·  "
     "Voice-enabled agents via Azure Cognitive Services  ·  "
     "Full audit trail for regulated industries  ·  Self-hosted — no SaaS lock-in."),
    (C_PURPLE_ACC, "Competitive Edge",
     "The only platform combining: Visual Canvas + Azure-native + Self-hosted + Safety Rules + "
     "Voice Agents + Simulation + Evaluations. "
     "Beats Flowise, Langflow, Dify, CrewAI, Lyzr, and MS Copilot Studio on all 7 dimensions."),
]
...
# ════════════════════════════════════════════════════════════════
# SLIDE 3 — What is AgentForge (Project Description)
# ════════════════════════════════════════════════════════════════
...
desc_items = [
    ...
    ("Prompt-to-Agent AI",
     "Describe your agent in one English sentence — GPT-4o instantly generates the agent name, "
     "system prompt, recommended tools, and guardrail settings automatically.",
    ...
    ("Enterprise Control Plane",
     "Live stats dashboard, agent versioning, full audit trail, RBAC (Admin/Developer/Viewer), "
     "OpenTelemetry tracing to Jaeger/Azure Monitor/GCP/Datadog.",
     RGBColor(0x16, 0x52, 0x9a)),
]
...
box(sl, 0.75, 6.08, 12, 0.42, "AgentForge solves all three",
    size=16, bold=True, color=C_WHITE, font=FONT_HEADING)
box(sl, 0.75, 6.5, 12, 0.42,
    "Visual builder  ·  8 Safety rules  ·  Evaluations engine  ·  Voice agents  ·  Full observability",
...
big_nums = [
    ...
    ("$0",   "Extra Licensing\nfor Guardrails",RGBColor(0x0c, 0x5e, 0x48), C_GREEN_ACC),
...
roi_cards = [
    ...
    ("Voice Agent Capability",
     "Azure Cognitive TTS with 8 Neural voices built-in.\n"
     "No separate telephony vendor required.\n"
     "Inbound & outbound call workflows from day one.",
     RGBColor(0x4a, 0x30, 0xb0)),
    ("Quality Assurance",
     "Evaluation engine scores agents pre-deployment.\n"
     "Only agents meeting pass-rate threshold promote.\n"
     "Prevents production incidents from agent drift.",
...
layers = [
    (6.5,  0.65, RGBColor(0x04, 0x18, 0x34), RGBColor(0x16, 0x52, 0x9a),
     "AZURE AI LAYER",
     "Azure OpenAI GPT-4o  ·  Azure OpenAI GPT-4.5  ·  Azure AI Search  ·  Azure Cognitive Services Speech (TTS/STT)"),
    (5.72, 0.7,  RGBColor(0x02, 0x1c, 0x14), RGBColor(0x0c, 0x5e, 0x48),
     "DATA LAYER",
     "PostgreSQL 16 + pgvector  ·  Alembic Migrations  ·  SQLAlchemy 2.0 Async ORM"),
    (4.42, 1.22, RGBColor(0x10, 0x06, 0x2c), RGBColor(0x4a, 0x30, 0xb0),
     "CORE SERVICES",
     "AgentOrchestrator (single + multi-agent)  ·  GuardrailsEngine (Presidio PII + Hallucination)  ·  RAGEngine (LangChain)  ·  "
     "SimulationRunner  ·  ToolRegistry (7 tools)  ·  PromptToAgent  ·  SafetyRules (8 rules)  ·  EvaluationEngine  ·  VoiceService (Azure Speech)"),
    (3.28, 1.06, RGBColor(0x22, 0x0e, 0x00), RGBColor(0x78, 0x46, 0x08),
     "API LAYER — FastAPI + JWT RBAC",
     "Agents  ·  Auth  ·  RAG  ·  Simulation  ·  Tools  ·  Control Plane  ·  Safety  ·  Evaluations  ·  Voice  ·  Knowledge Base"),
    (2.16, 1.04, RGBColor(0x06, 0x12, 0x26), RGBColor(0x5b, 0xa3, 0xf5),
     "FRONTEND — React 18 + Vite + TypeScript",
     "ReactFlow Canvas  ·  Agent Studio (type filters)  ·  Safety & Guardrails  ·  Evaluations  ·  Voice Agents  ·  Dashboard  ·  Zustand  ·  TailwindCSS"),
]
...
rect(sl, 11.22, 2.16, 1.66, 5.0, RGBColor(0x02, 0x14, 0x0c),
     border_color=RGBColor(0x18, 0x7a, 0x5e))
box(sl, 11.28, 2.22, 1.52, 0.32, "OBSERVABILITY",
    size=9, bold=True, color=C_TEAL_ACC, font=FONT_HEADING)
for i, item in enumerate(["OpenTelemetry", "Jaeger", "Azure Monitor", "GCP Trace", "Datadog APM"]):
    box(sl, 11.28, 2.60 + i*0.68, 1.48, 0.58, item, size=11, color=C_OFF_WHITE, font=FONT_BODY)
...
# ════════════════════════════════════════════════════════════════
# SLIDE 7 — NEW: Safety & Guardrails Feature
# ════════════════════════════════════════════════════════════════
sl = add_slide(C_DARK_BG)
top_bar(sl, C_RED_ACC, 0.08)
box(sl, 0.55, 0.28, 8, 0.42, "NEW FEATURE SPOTLIGHT", size=12, bold=True,
...
# ════════════════════════════════════════════════════════════════
# SLIDE 8 — NEW: Evaluations Engine
# ════════════════════════════════════════════════════════════════
sl = add_slide(C_WHITE_BG)
box(sl, 0.55, 0.28, 6, 0.42, "NEW FEATURE SPOTLIGHT", size=12, bold=True,
    color=C_MUTED_TXT, font=FONT_HEADING)
...
# ════════════════════════════════════════════════════════════════
# SLIDE 10 — Agent Studio with Type Filters
# ════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════
# SLIDE 11 — vs Lyzr Architect (Detailed)
# ════════════════════════════════════════════════════════════════
sl = add_slide(C_WHITE_BG)
...
headers = ["Capability", "AgentForge  ✅", "Lyzr Architect"]
rows_lyzr = [
    ...
    ["Pre-production simulation",     "✅ Pass-rate scoring",   "❌"],
]
tbl(sl, 0.55, 1.58, 12.23, 5.6, headers, rows_lyzr, col_widths=[5.2, 3.8, 3.2])
...
box(sl, 0.72, 7.1, 12, 0.3,
    "AgentForge adds Evaluations, Voice Agents, and 8-rule Safety dashboard that Lyzr does not offer.",
    size=13, bold=True, color=RGBColor(0x16, 0x52, 0x9a), font=FONT_BODY)


# ════════════════════════════════════════════════════════════════
# SLIDE 12 — Market Comparison (Broad)
# ════════════════════════════════════════════════════════════════
...
headers2 = ["Tool", "Canvas", "Guardrails", "Evaluations", "Voice", "Azure-Native", "Self-Host", "Simulation"]
rows_market = [
    ["AgentForge",        "✅", "✅", "✅", "✅", "✅", "✅", "✅"],
...
tbl(sl, 0.55, 1.58, 12.23, 5.3, headers2, rows_market,
    col_widths=[2.4, 1.28, 1.52, 1.52, 1.2, 1.52, 1.28, 1.51])

r_note = sl.shapes.add_shape(1, Inches(0.55), Inches(7.05), Inches(12.23), Inches(0.4))
r_note.fill.solid(); r_note.fill.fore_color.rgb = RGBColor(0xe0, 0xec, 0xff)
r_note.line.color.rgb = RGBColor(0x16, 0x52, 0x9a); r_note.line.width = Pt(0.75)
box(sl, 0.72, 7.08, 12, 0.32,
    "AgentForge is the only platform with Canvas + Guardrails + Evaluations + Voice + Azure-Native + Self-Host + Simulation — all 7.",
    size=13, bold=True, color=RGBColor(0x16, 0x52, 0x9a), font=FONT_BODY)


# ════════════════════════════════════════════════════════════════
# SLIDE 13 — Full Feature List
# ════════════════════════════════════════════════════════════════
sl = add_slide(C_DARK_BG)
top_bar(sl, RGBColor(0x16, 0x52, 0x9a))
box(sl, 0.55, 0.28, 5, 0.42, "COMPLETE FEATURE SET", size=12, bold=True,
    color=C_BLUE_ACC, font=FONT_HEADING)
box(sl, 0.55, 0.68, 12, 0.78, "17 Enterprise Features Shipped",
    size=30, bold=True, color=C_WHITE, font=FONT_HEADING)
...
for i, (name, desc, acc) in enumerate(all_feats):
    col = i % 3; row = i // 3
    cx = 0.55 + col*4.27; cy = 1.72 + row*1.08
    r2 = sl.shapes.add_shape(1, Inches(cx), Inches(cy), Inches(4.1), Inches(0.98))
    ...
    a.fill.solid(); a.fill.fore_color.rgb = acc; a.line.fill.background()
    box(sl, cx+0.14, cy+0.06, 3.8, 0.34, name, size=13, bold=True,
        color=C_WHITE, font=FONT_HEADING)
    box(sl, cx+0.14, cy+0.44, 3.8, 0.46, desc, size=12, color=C_OFF_WHITE, font=FONT_BODY)


# ════════════════════════════════════════════════════════════════
# SLIDE 14 — Quality & Delivery Metrics
# ════════════════════════════════════════════════════════════════
sl = add_slide(C_BLUE_BG)
...
for i, ep in enumerate(endpoints):
    box(sl, 0.72, 4.1 + i*0.44, 5.8, 0.4, ep, size=12, color=C_OFF_WHITE,
        font=FONT_CODE)
...
sec_items = [
    ...
    "JWT enforced on all 9 router modules",
]
for i, s in enumerate(sec_items):
    box(sl, 7.0, 4.1 + i*0.44, 5.7, 0.4, f"✓  {s}", size=12,
...
cases = [
    ...
    ("Voice-Enabled Customer Support",
     "Voice agent uses Azure Neural TTS to answer calls in natural language. "
     "Per-agent voice selection, silence detection, and max call duration configured in UI.",
     C_PURPLE_ACC),
...
info_pills = [
    ...
    ("Version:  3.0  —  July 2026",                               RGBColor(0x01, 0x1e, 0x14), C_TEAL_ACC),
    ("Features:  17  ·  Tests:  32/32",                           RGBColor(0x01, 0x1e, 0x14), C_GREEN_ACC),
]
...
# SAVE
# ════════════════════════════════════════════════════════════════
out = r"C:\Users\n.sureshmanikandan\Repo1\AgentForge\docs\AgentForge-PresentationV3.0.pptx"
prs.save(out)
print(f"\nSaved: {out}")
